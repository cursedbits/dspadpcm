...
/*--------------------------------------------------------------------------*/
void adpcmCoefTableDesign(s16* input_, u32 nsamples, ADPCMINFO* cxt)
{
	double  thresh          = 10.0;
	int     order           = ORDER;
	int     refine_iter     = 2;
			...
			if(fabs(ac[0]) > thresh) {
				...
				if(!lud(a, order, indx, &d)) {
					...
					if(!kfroma(ac, ref, order)) {
						...
						/* printf("a1:%f  a2:%f ", training[aframecnt][1], training[aframecnt][2]); */
						aframecnt++;
					}
				}
			}
	...
	/* To start things off find the average auto-correlation over the
	 * complete data set. */
	ac[0] = 1.0;
	...
	while(actual_size < log2_tablesize) {
		...
		/* Split each codebook template into two - the original and a
		 * shifted version */
		for(i = 0; i <= order; i++) {
			dir[i] = 0;
		}
