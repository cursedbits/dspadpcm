...
{
	u32 frames      = nibble / 16;
	u32 nibblesLeft = nibble % 16;
 ...
 *---------------------------------------------------------------------------*/
void encode(
	s16*		src,     /* location of source samples (16bit PCM signed
				    little endian) */
	u8*		dst,     /* location of destination buffer */
	ADPCMINFO*	cxt,     /* location of adpcm info */
	u32		samples) /* number of samples to encode */
{
	adpcmCoefTableDesign(src, samples, cxt);

	encodeLoop(src, samples, dst, cxt);
